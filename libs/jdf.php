...
function jalali_to_gregorian_converter($string, $type) {
    if ($type == 'date') {
        $date = explode("-", $string);
        $new_date = jalali_to_gregorian($date['0'], $date['1'], $date['2']);
        $jdate = "{$new_date['0']}-{$new_date['1']}-{$new_date['2']}";
        return $jdate;
    }
    if ($type == 'date_time') {
        $date_time = explode(" ", $string);
        $date = explode("-", $date_time['0']);
        $new_date = gregorian_to_jalali($date['0'], $date['1'], $date['2']);
        $jdate = "{$new_date['0']}-{$new_date['1']} {$new_date['2']}" . ' ' . $date_time['1'];
        return $jdate;
    }
}

function gregorian_to_jalali_converter($string, $type) {
    if ($type == 'date_main') {
        $date = explode("-", $string);
        $new_date = gregorian_to_jalali($date['0'], $date['1'], $date['2']);
    ...
    if ($type == 'date') {
        $date = explode("-", $string);
        $new_date = gregorian_to_jalali($date['0'], $date['1'], $date['2']);
        $month_name = jdate_words(array('mm' => $new_date['1']), ' ');
        $jdate = "{$new_date['2']} {$month_name} {$new_date['0']}";
        return $jdate;
    }
    if ($type == 'date_time') {
        $date_time = explode(" ", $string);
        ...
        $jdate = "{$new_date['2']} {$month_name} {$new_date['0']}" . ' ' . $date_time['1'];
        return $jdate;
    }
}
